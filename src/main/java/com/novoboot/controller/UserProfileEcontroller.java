...
public class UserProfileEcontroller {
	...
	public ResponseEntity<?> getPreviousBookingService(@PathVariable("detailFor") String detailFor,@PathVariable("userId") int userId) {
		...
		if(detailFor.equals(BASIC_STRINGS.SERVICE.getStringName())) {
			...
			List<UserPackageBookingDetails> packageBookingDetails= userProfileService.getPreviousPackageBookingService(userId);
...
//		return null;
	...
	@RequestMapping(value = "/updateServiceAddress", method = RequestMethod.POST)
	public ResponseObject updateAddress(@RequestBody String request ) {
